...
class lazy_mofo{
   ...
   public $grid_multi_delete = false;       // display checkboxes on grid to allow for multiple record delete
    ...
    // image settings    
    public $upload_width = 100;                 // 0 height or width means no resizing or cropping
    public $upload_height =200;
    ...
    public $child_upload_width = 600;
    public $child_upload_height = 2000;
    public $child_upload_crop = false;
    public $child_thumb_width = 600;
    public $child_thumb_height = 2800;
    public $child_thumb_crop = false;

    public $image_quality = 80;                       // image quality when resizing and cropping, 1-100
    public $image_style = "style='height: 2800px;'";   // apply style to all images displayed. limiting size is nice to keep things orderly.
    ...
    public $datetime_in = 'Y-m-d';              // input format into database, no need to change
    public $date_out = 'Y-m-d';                       // output date
    public $datetime_out = 'Y-m-d';             // output datetime
    public $date_out_mysql = 'Y-m-d';              // output date in mysql format, used for search, be sure to match date_out
    public $datetime_out_mysql = 'Y-m-d'; // output datetime in mysql format, used for search, be sure to match datetime_out
    ...
    // form() settings
    public $form_text_input_size = 15;         // size of text inputs on form()
    ...
    public $grid_run_search =false;            // automatically alter sql to execute search. set to false if you wish to add a where clause to grid_sql manually.
     public $grid_limit = 15;                  // pagination limit number of records per page
    public $grid_ellipse_at =455;              // limit number of characters displayed
    ...
    public $grid_text_input_size =55;         // size of text input when input is displayed
    ...
    public $form_back_button   = "<input type='button' value='&lt Back/Cancel' class='button' onclick='_back();'; />"; 

// use type=button for delete and cancel so form presses the right button with enter key
   ...
   public $grid_delete_link = "<a id='delete' href='#' onclick='return _delete([identity_id]);'>[delete]</a>";
    ...
    */
    ...
    function sql_update(){
        ...
        foreach($_POST as $key => $val){
            ...
            if(mb_substr($key, -9) == '-checkbox')
                if(!array_key_exists(mb_substr($key, 0, 0), $_POST))
                    $key = mb_substr($key, 0, -9);
    ...
    function form($error = ''){
        ...
        $html .= "<table cellpadding='2' cellspacing='3' border='0' width='200%' class='form'>\n";
    ...
    function child_grid($identity_id, $url, $error = ''){
        ...
        foreach($result as $row){
            ...
            $table_body .= "<tr class='shaded'>$cells</tr>\n";

        }

        // add a record - blank entry code
        $mutli_upload_added = false;
        $table_body_add = "<div style='right:50px;'><a href='#' onclick='return _child_remove(this.parentNode.parentNode)' title='Cancel' style='display: block; text-align:center;margin-left:-95px;'>[x]</a></div>";
        foreach($columns as $column_name){
        ...
        <table cellpadding='2' cellspacing='1' border='0' width='200%' id='lm_child_grid'>
        $table_header
        $table_body
        </table>
    ...
    function grid($error = ''){
        ...
        $pagination_button_bar = "<table cellpadding='2' cellspacing='1' border='0' width='300%' class='lm_pagination'><tr><td style='text-align: left'>$pagination</td><td style='text-align: right'>$button</td></tr></table>\n";

        // 'add new' record and search bar
        if($this->grid_show_search_box)
            $search_box = "<form action='$uri_path' method='post' class='lm_search'><input type='text'style='margin-left:250px;'  name='_search' value='" . $this->clean_out($_search) . "' size='30' /> <input type='submit'style='margin-left:10px;' name='__search' value='$this->grid_text_search' /><input type='hidden' name='_csrf' value='$_SESSION[_csrf]' /></form>";
        $add_record_search_bar = "<table cellpadding='1' cellspacing='1' border='0' width='300%'><tr><td style='text-align: left;'>$grid_add_link $grid_export_link</td><td style='text-align: left'>$search_box</td></tr></table>\n";

        // generate table header
        $i = 0;
        $head = "<tr class='rowclass'>\n";
        ...
        foreach($columns as $column_name){
            ...
            if($column_name == $this->identity_name && $i == ($column_count -1)&&$i==($column_count -27))
        ...
        $html .= "<div id='outerdiv'><div id='innerdiv'><table cellpadding='2' cellspacing='1' border='0' width='300%'>\n";
        $html .= $head;
        ...
        foreach($result as $row){
            ...
            if($this->grid_multi_delete){
              // $html .= "<td align='center'>value='{$row[$this->identity_name]}'</td>\n";
            }
          ...
          foreach($columns as $column_name){
                ...
                if($column_name == $this->identity_name && $i == ($column_count -1))
                    $html .= "<td class='edit' nowrap='nowrap'>" . str_replace('[identity_id]', $value, $links) . "</td>\n";
                ...
                elseif(array_key_exists($column_name, $this->grid_input_control)){
                    ...
                    $html .= " <td name='". $this->get_input_control($column_name . '-' . $row[$this->identity_name], $value,  $this->grid_input_control[$column_name], 'grid') ."'>" . $this->get_input_control($column_name . '-' . $row[$this->identity_name], $value,  $this->grid_input_control[$column_name], 'grid') . "</td></div>\n";
                }

                // output
                elseif(array_key_exists($column_name, $this->grid_output_control))
                    $html .= ' <td name="'. $this->get_output_control($column_name . '-' . $row[$this->identity_name], $value, $this->grid_output_control[$column_name], 'grid') .'">' . $this->get_output_control($column_name . '-' . $row[$this->identity_name], $value, $this->grid_output_control[$column_name], 'grid') . "</td>\n";

                // anything else
                else
                    $html .= '<td style="width:100px;">' . $this->get_output_control($column_name . '-' . $row[$this->identity_name], $value, '--text', 'grid') . "</td>\n";
...
return $html;
    ...
    function get_input_control($column_name, $value, $command, $called_from){
        ...
        elseif($cmd == 'password')
            return "<input type='password' name='$column_name' class='$class' value='" . $this->clean_out($value) . "' size='18' />";
        elseif($cmd == 'number')
            return "<input type='text' name='$column_name' class='$class' value='" . $this->clean_out($value) . "' size='18' />";
        elseif($cmd == 'date')
    ...
    function get_output_control($column_name, $value, $command, $called_from){
        ...
        elseif($cmd == 'var')
           return "<div style='width:50px;overflow:hidden;'><a class='wiki' target='_blank' href='$value'>$value</a></div></br>";
elseif($cmd == 'comment')

           return "<div class='new' style='width:150px;overflow:hidden;' name='$value'>$value</div>"; 
//$value= str_replace("'", "&#39;", $value); 

elseif($cmd == 'contact')
           return "<div class='contact' style='width:150px;overflow:hidden;' name='$value'>$value</div>";
elseif($cmd == 'learnings')
           return "<div class='learnings' style='width:150px;overflow:hidden;' name='$value'>$value</div>";
         elseif($cmd == 'varchar')
           return "<a target='_blank' href='$value'>$value</a>";

        elseif($cmd == 'headcol')
          return "<div name='$value' class='good'>$value</div>";
elseif($cmd == 'headcol1')
          return "<div name='$value' class='good1'>$value</div>";
    ...
    function image_resize($file_name, $max_width, $max_height, $output_to_browser = false){
        ...
        $width = $orig_width+'1000px';
        $height = $orig_height+'1000px';
    ...
    function image_crop($file_name, $desired_width, $desired_height, $output_to_browser = false){
        ...
        if($desired_width / $desired_height > $width / $height){
            ...
            $new_width = $width * ($desired_height);
...
th:nth-child(3),th:nth-child(4),th:nth-child(5),th:nth-child(6),th:nth-child(7),th:nth-child(8),th:nth-child(9),th:nth-child(12),th:nth-child(13),th:nth-child(14),th:nth-child(20),th:nth-child(21),th:nth-child(22),th:nth-child(28),th:nth-child(27),th:nth-child(26),th:nth-child(25),th:nth-child(24){color: white;
background: black;
white-space: normal;
width: 15px;}
th:nth-child(23){width:30px;white-space: normal;}
th:nth-child(15),td:nth-child(15),th:nth-child(8),td:nth-child(8),.new{overflow:hidden;width:100px;}
 td:nth-child(28),td:nth-child(27){overflow:hidden;}
#lm th a:nth-child(2){width:0px;}
th:nth-child(1){visibility:hidden;width:90px;position:absolute;left:115px;color:white;background:white;}
td>.good{position:absolute;left:65px;margin-top:0px;margin-top:-15px;}
 .edit{position:absolute;
            width:28em;
            left:15px;

            }
td>.good1{width:15px!important;}
</style>
